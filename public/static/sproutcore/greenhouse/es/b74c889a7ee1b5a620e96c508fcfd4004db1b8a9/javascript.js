...
return SC.DRAG_NONE},treeItemChildrenKey:"contents",reload:function(){var b=Greenhouse.store.find(Greenhouse.FILES_QUERY),a=SC.Object.create({treeItemIsExpanded:YES});
a.set("contents",b);this.set("content",a)}});Greenhouse.layoutController=SC.ObjectController.create({contentBinding:"Greenhouse.pageController.designController.selection",allowsMultipleContent:YES,hDimNowShowing:function(){var c=this.get("anchorLocation"),a=SC.ViewDesigner,b="leftDimensions";
...
}}return c},listFiles:function(a,b){SC.Request.create({type:"GET",isJSON:YES,address:"/sproutcore/fs?action=list"}).notify(this,this.listFilesDidComplete,{query:b,store:a}).send()
...
a.loadQueryResults(f,c);Greenhouse.sendAction("fetchTargetsDidComplete")}},updateRecord:function(a,d,e){var b=a.materializeRecord(d);
...
}else{this.showDropDown()}}};require("core");Greenhouse.Dir=SC.ChildRecord.extend({type:"Dir",childRecordNamespace:Greenhouse,name:SC.Record.attr(String),dir:SC.Record.attr(String),contents:SC.Record.toMany("SC.Record",{nested:YES}),primaryKey:"id",isFile:NO,path:function(){return this.get("dir")+this.get("name")
...
Greenhouse.TARGETS_QUERY=SC.Query.remote(Greenhouse.Target);Greenhouse.mixin({inspectorClosed:SC.State.create({parallelStatechart:"inspector",openInspector:function(a){if(a){Greenhouse.openInspectorPicker.set("anchor",a)
