...
describe User do
  ...
  it "should belong to certain roles" do
    u = standard_user
    login = u.login
    u.save
    ...
    User.first.has_role?(role_name).should eql(true)
  ...
  it "should allow many users to belong to many roles" do
    ...
    User.get(u1.id).roles.length.should eql 1
    ...
    User.get(u1.id).roles.length.should eql 1
    User.get(u2.id).roles.length.should eql 1
    User.get(u3.id).roles.length.should eql 1
  end
