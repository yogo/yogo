...
class His::DataValuesController < ApplicationController
  ...
  def show
    @data_value = His::DataValue.get(params[:id])
  ...
  def create
    @data_value = His::DataValue.new(params[:new_data_value])

    respond_to do |format|
      if @data_value.save
        flash[:notice] = 'His::DataValue was successfully created.'
  ...
  def update
    ...
    respond_to do |format|
      if @data_value.update_attributes(params[:data_value])
        flash[:notice] = 'His::DataValue was successfully updated.'
        format.html { redirect_to(raw_data_value_path(@data_value.id)) }
      else
