...
class Yogo::AnonymousUser
  ...
  def method_missing(method, *args)
    if eval(Yogo::Setting[:user_class]).new.respond_to?(method)
      raise Yogo::UserMethodOnAnonymousUser.new("undefined method '#{method}' for #{self}. Did you mean to call it on a #{Yogo::Setting[:user_class]} class instead?")
    else
