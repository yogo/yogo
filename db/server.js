(function(){
	...
	var tempQuery = function(/*String*/query, args){// persevere edit
		...
		var __ids__={};
		var idNum = 0;
		// This needs to be /g (globally applied for compound tests, e.g. [?foo=/foo/1&bar=/bar/2] -- IRJ
		// (OLD) query = query.replace(/\/([\w_$]+\/[\w]+)/g,function(t,a){ This change makes namespacing in .contains work correctly. -- IRJ
		query = query.replace(/((\/[\w_$]+)+(\/[\w]+))/g,function(t,a){
		...
		query = query.replace(/([^=<>]=)([^=])/g,"$1=$2"). // change the equals to comparisons
                        // This replace needs the 0-9 so that properties can contain numbers (and more than one digit, e.g. street12 or b8te).
			replace(/@|(\.\s*)?[a-zA-Z\$_][a-zA-Z0-9\$_]+(\s*:)?/g,function(t){ 
